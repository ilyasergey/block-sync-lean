...
| Document | What it is |
|---|---|
| [docs/paper-additions-stage1.md](docs/paper-additions-stage1.md) | **Suggested paper additions — Stage 1.** Brief, paper-terminology-only list of recommended edits, organized in the order theorems appear. Covers only items whose Lean proofs are fully closed at the time of writing, plus a paper-language statement of the Theorem 7 obstruction (T7 cannot be closed as currently stated). **Audience:** paper authors. **Use when:** assembling a single round of feedback to send to the authors. **Feeds:** paper feedback. Will be extended in later stages as more proofs land. |
| [docs/paper-additions-stage2.md](docs/paper-additions-stage2.md) | **Suggested paper additions — Stage 2.** Companion to Stage 1, recording items surfaced after Stage 1 was written. Currently one item: qualifying the BFT bound's universe of quantification (refinement of finding F-8). **Audience:** paper authors. **Use when:** sending a follow-up round of feedback. **Feeds:** paper feedback. |
| [docs/mechanization-findings.md](docs/mechanization-findings.md) | **Running log of paper-side observations from the formalization.** Each finding (`F-1` … `F-11`, plus sub-findings like `F-1a`) has severity, category, affected paper sections, headline issue, recommended action, and our Lean-side address. **Audience:** paper authors, ourselves when proposing edits. **Use when:** mechanization surfaces a paper concern (missing assumption, hidden invariant, off-by-one, scope ambiguity). **Feeds:** paper feedback — superset of the per-stage Stage-N docs (also includes findings tied to in-flight or hypothesis-only proofs). Paper terminology only; no Lean. |
