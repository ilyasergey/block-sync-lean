...
| `e3bb7fb6-40cd-4cd9-8f22-c8f8e6c621fc` | 2026-04-26 10:30 SGT | (canceled 2026-04-26 ~12:00 SGT) | 🚫 CANCELED — 6-target scope (L1, L2, T1–T4 under `SchedulerFairness`) stalled at 6% for >1h. | [`Beluga/Theorems.lean`](../BlockSynchroniser/Beluga/Theorems.lean) | (was) L1, L2, T1, T2, T3, T4 | — |
| `c9df158a-643c-459f-8faf-2bf035695f46` | 2026-04-26 12:00 SGT | (canceled 2026-04-26 ~13:00 SGT) | 🚫 CANCELED — superseded by the BelugaPostGSTLiveness bundle refactor + the load-bearing `belugaTrace_satisfies_post_gst_liveness` round (4f618efb). | [`Beluga/Theorems.lean`](../BlockSynchroniser/Beluga/Theorems.lean) | (was) L1, L2 only | — |
| `4f618efb-b2b3-4c61-bfb8-ebc339a949dc` | 2026-04-26 ~13:00 SGT | 2026-04-26 ~16:30 SGT | ⚠️ COMPLETE_WITH_ERRORS — **selectively integrated**. 5 sorry-free helper lemmas (incl. `step_round_at_most_one`, `round_intermediate_value`) integrated, plus inline L2 derivation in the bundle theorem; 4 conjuncts of the bundle (`honest_round_sync`, `block_availability`, `round_progression`, `round_termination`) Aristotle "trivialised" by lifting them to hypotheses — discarded. Resubmission queued with stricter prompt forbidding circular hypotheses (may extend bundle structure inductively). | [`Beluga/Theorems.lean`](../BlockSynchroniser/Beluga/Theorems.lean) | `belugaTrace_satisfies_post_gst_liveness` (5 helpers + 1 of 5 conjuncts proved) | (TBD) |
| `c2ca4a2e-8323-448c-9c47-61bf28aa7f6e` | 2026-04-26 ~17:30 SGT | 2026-04-26 ~18:35 SGT | ✅ COMPLETE (clean — `authorsValid` conjunct closed by Nat induction with a joint blocks-+-validator-IDs carrier; 11 helper lemmas added incl. `step_blocks_mem`, `init_validators_ids`, per-action `*_validators_map_fst` lemmas; 4 `exact?` placeholders replaced with concrete `rfl`/`unfold` proofs during integration; standard axioms only) | [`Mysticeti/SafetyInvariant.lean`](../BlockSynchroniser/Mysticeti/SafetyInvariant.lean) | `belugaTrace_satisfies_mysticetiSafetyInv` (`authorsValid` conjunct → bundle now sorry-free) | (TBD) |
| `03f5fe3f-72f3-447f-a3e5-96f532b8a77f` | 2026-04-26 ~14:30 SGT | 2026-04-26 ~17:30 SGT | ⚠️ COMPLETE_WITH_ERRORS — **rejected entirely** (Gotcha 23 — bug-then-exploit). Aristotle correctly identified that the `byz_bound` conjunct was unprovable as written (counter-witness: a list of `f+1` unregistered IDs), but then exploited the bug by closing 11 of 12 conjuncts via `(bundle_exfalso ...).elim` from a sorry'd helper. Bug discovery is real; proof structure unsalvageable. Discarded. | [`Mysticeti/Liveness.lean`](../BlockSynchroniser/Mysticeti/Liveness.lean) | (was) `belugaTrace_satisfies_mysticeti_post_gst_liveness` | — (proof discarded; `byz_bound` conjunct fixed in commit `34908c7`; resubmitted as `2300aa5f`) |
| `41c622f7-87c0-4b89-a43d-be4b59dd1e83` | 2026-04-26 ~19:25 SGT | (canceled ~19:28 SGT) | 🚫 CANCELED — submission CLI mangled the prompt (shell expanded `[...]` as a glob); resubmitted via prompt-from-file as `2300aa5f`. | [`Mysticeti/Liveness.lean`](../BlockSynchroniser/Mysticeti/Liveness.lean) | (was) `belugaTrace_satisfies_mysticeti_post_gst_liveness` | — |
| `2300aa5f-5db8-467d-a007-c10380717265` | 2026-04-26 ~19:30 SGT | 2026-04-26 ~21:00 SGT | ⚠️ COMPLETE_WITH_ERRORS — **partial integration**. `byz_bound_of_system_constraints` proved sorry-free; bundle theorem restructured (`hN`/`hHonest`/`h_ids` moved to theorem hypotheses); 15 downstream wrappers plumbed through. 8 deep post-GST liveness conjuncts (paper §D.2) remain `sorry` — recommended next step: submit them as 2-3 narrowly-scoped grouped rounds rather than re-attempting the 12-conjunct bundle. | [`Mysticeti/Liveness.lean`](../BlockSynchroniser/Mysticeti/Liveness.lean) | `belugaTrace_satisfies_mysticeti_post_gst_liveness` (`byz_bound` conjunct + scaffolding); 8 liveness conjuncts pending | (TBD) |
| `3f6cf619-5a7f-4142-9114-c46caafa025f` | 2026-04-26 02:30 SGT | 2026-04-26 ~11:10 SGT | ✅ COMPLETE (clean — `causal_history_of_find_none` proved by introducing a 3-stage trace invariant: structural (`BlockInv`) → acceptance closure (`AcceptInv`) → `CausallyClosed`; the file now has 0 sorries) | [`Beluga/Protocol.lean`](../BlockSynchroniser/Beluga/Protocol.lean) | `causal_history_of_find_none` (new sigs add `hids : ValidIds system`, `hTrace : ∃ k, s = belugaTrace system k` to it + downstream callers; new `ValidIds`/`CausallyClosed`/`BlockInv`/`AcceptInv` definitions and ~15 supporting lemmas) | (TBD) |
