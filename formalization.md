...
| §4 — Protocol semantics: `HonestStep` (relational) + executable `step` + `belugaTrace` | [`Beluga/Protocol.lean`](BlockSynchroniser/Beluga/Protocol.lean#L192) | ◐ Definitions ✅; `step_refines_HonestStep` proved at the top level but transitively depends on a trace-level invariant `causal_history_of_find_none` (about ancestor-accepted accumulation across the trace), still pending |
...
| **Lemma 8** — leader block referenced next round (after GST) | [`Mysticeti/Liveness.lean :: lemma8_leader_referenced`](BlockSynchroniser/Mysticeti/Liveness.lean#L104) | ◐ Top-level composition closed; transitive sorries on atomic protocol invariants (step semantics + parent-selection) pending |
| **Lemma 9** — honest validators create certificate for honest leader | [`Mysticeti/Liveness.lean :: lemma9_honest_certificate`](BlockSynchroniser/Mysticeti/Liveness.lean#L158) | ◐ Top-level composition closed; transitive sorries pending |
| **Lemma 10** — round-robin pigeonhole (3 consecutive honest leaders in any 3f+3 window) | [`Mysticeti/Safety.lean :: lemma10_round_robin_pigeonhole`](BlockSynchroniser/Mysticeti/Safety.lean#L106) | ✅ proved (takes `n=3f+1`, honest-count = 2f+1, validator-IDs-contiguous; the last is finding F-8). |
| **Lemma 11** — undecided leader block eventually decided | [`Mysticeti/Liveness.lean :: lemma11_eventual_decision`](BlockSynchroniser/Mysticeti/Liveness.lean#L247) | ◐ Top-level composition closed; transitive sorries on backward-induction sub-step pending |
| **Lemma 12** — block referenced by 2f+1 ⇒ honest validators output `block_accept` | [`Mysticeti/Liveness.lean :: lemma12_referenced_accepted`](BlockSynchroniser/Mysticeti/Liveness.lean#L308) | ◐ Top-level composition closed; transitive sorries on Byzantine-bound + ImPoA-availability pending |
| **Lemma 13** — certificate persistence across rounds | [`Mysticeti/Safety.lean :: lemma13_cert_persistence`](BlockSynchroniser/Mysticeti/Safety.lean#L205) | ✅ proved via paper §D.3 quorum-intersection argument; uses the new `AdmissionWellFormed` trace invariant (a *theorem* about `belugaTrace`, not a hypothesis) plus `h_honest_unique` (standard BFT no-equivocation). Closes F-5 item 1. |
| **Lemma 14** — no honest validator skips a directly-committed leader | [`Mysticeti/Safety.lean :: lemma14_no_skip`](BlockSynchroniser/Mysticeti/Safety.lean#L287) | ✅ proved. |
| **Lemma 15** — at most one certified leader per round | [`Mysticeti/Safety.lean :: lemma15_unique_cert`](BlockSynchroniser/Mysticeti/Safety.lean#L316) | ✅ specialization of `certified_unique`; threads through the same BFT side conditions. |
| **Lemma 16** — consistent leader-status decision across honest validators | [`Mysticeti/Safety.lean :: lemma16_consistent_status`](BlockSynchroniser/Mysticeti/Safety.lean#L373) | ✅ proved with `h_view_traceback` hypothesis; see finding F-5. |
