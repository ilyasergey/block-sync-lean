...
| §5 — **Lemma 2** — round-to-round latency ≤ 3Δ in the happy case | [`Beluga/Theorems.lean :: lemma2_round_latency`](BlockSynchroniser/Beluga/Theorems.lean#L555) | ✅ **proved** (derived inline from the lockstep `SchedulerFairness` + `round_intermediate_value`); the bundle is now sorry-free. |
| §5 — **Theorem 1** — Beluga ⊨ Block availability | [`Beluga/Theorems.lean :: theorem1_block_availability`](BlockSynchroniser/Beluga/Theorems.lean#L571) | ✅ **proved** via `step_advance_implies_stored` (store-before-advance gate of `tryActFor`) + `find_advance_step` + `acceptInv_trace` (the `acceptedBlockExists` direction). One-line projection of the bundle, whose T1 conjunct is now sorry-free. |
...
| §5 — **Theorem 4** — Beluga ⊨ Round-Termination | [`Beluga/Theorems.lean :: theorem4_round_termination`](BlockSynchroniser/Beluga/Theorems.lean#L620) | ✅ **proved** via `round_termination_aux` (accept-before-advance gate + `accepted_at_advance` strong induction on round + `block_parents_in_pool` + `belugaTrace_proposeOp_in_pool` for author identification via `BlockInv` canonical+`digest_injective`). |
| §5 — **Bundle**: `BelugaPostGSTLiveness` packaging L1, L2, T1, T3, T4 | [`Beluga/Theorems.lean :: BelugaPostGSTLiveness`](BlockSynchroniser/Beluga/Theorems.lean#L437) | ✅ **All conjuncts proved.** `belugaTrace_satisfies_post_gst_liveness` is sorry-free. Bundle takes `hids : ValidIds system` (needed by T1's `acceptInv_trace` and T4's author-identification). |
| §5 corollary — Beluga is a block synchronizer (T1∧T2∧T3∧T4) | [`Beluga/Theorems.lean :: belugaTrace_isBlockSynchronizer`](BlockSynchroniser/Beluga/Theorems.lean#L634) | ✅ **proved** (conjunction `⟨T3, T4, T1, T2⟩`; bundle is now sorry-free). |
