...
| §5 network-aware fairness — same derivation against `networkTrace` (where ImPoA's f+1 references and `T_rd = 4Δ` timeout are encoded in `canAcceptBlock` / `networkTryActFor`) | [`Beluga/Network/Fairness.lean :: schedulerFairness_holds`](BlockSynchroniser/Beluga/Network/Fairness.lean#L770) | ✅ **proved**. ImPoA (paper §4.3) appears in `canAcceptBlock`'s `parentsAcceptableImPoA` clause, structurally consumed by `networkTryActFor_preserves_roundEntry_bound`'s case analysis. The `networkTrace` model and its structural invariants (id preservation, validator-list nodup, `roundEntryTime ≤ currentTime`) are all sorry-free. |
